...
describe('CuActionBar', () => {
  ...
  it('disabled action maps to disabled type', () => {
    const w = mount(CuActionBar, { props: { actions: [{ text: 'X', disabled: true }] } })
    expect(w.find('button').classes()).toContain('disabled')
  })

  it('click invokes action handler and emits click', async () => {
    const onClick = vi.fn()
    const w = mount(CuActionBar, { props: { actions: [{ text: 'go', onClick }] } })
  ...
  it('text slot renders and suppresses when empty', () => {
    const withText = mount(CuActionBar, {
      props: { actions: [{ text: 'go' }] },
      slots: { default: '<p>合计</p>' },
    })
    expect(withText.find('.cu-action-bar-text').exists()).toBe(true)
    const without = mount(CuActionBar, { props: { actions: [{ text: 'go' }] } })
    expect(without.find('.cu-action-bar-text').exists()).toBe(false)
  })
})

describe('CuDetailItem', () => {
  it('renders title and content with bold variant', () => {
    const w = mount(CuDetailItem, { props: { title: '标题', content: '内容', bold: true } })
    expect(w.find('.cu-detail-title').text()).toBe('标题')
    expect(w.find('.cu-detail-content').text()).toBe('内容')
    expect(w.classes()).toContain('is-bold')
  })

  it('slot overrides content', () => {
    const w = mount(CuDetailItem, { props: { title: 'T' }, slots: { default: 'SLOT' } })
    expect(w.find('.cu-detail-content').text()).toBe('SLOT')
  })
})

describe('CuTextareaItem', () => {
  it('v-model round-trips typed input', async () => {
    const w = mount(CuTextareaItem, { props: { modelValue: '', title: 'T' } })
  ...
  it('error prop renders message and is-error class', () => {
    const w = mount(CuTextareaItem, { props: { error: '错了', modelValue: 'x' } })
    expect(w.find('.cu-textarea-item-msg p').text()).toBe('错了')
    expect(w.find('.cu-textarea-item').classes()).toContain('is-error')
  })

  it('clear button appears on focus with value and clears', async () => {
    const w = mount(CuTextareaItem, { props: { clearable: true, value: '内容' } })
    // jsdom 下 VTU isVisible 受 offsetParent 限制，改用 v-show 的 style 断言
    expect(w.find('.cu-textarea-item__clear').attributes('style')).toContain('display: none')
    await w.find('textarea').trigger('focus')
    expect(w.find('.cu-textarea-item__clear').attributes('style')).not.toContain('display: none')
    await w.find('.cu-textarea-item__clear').trigger('click')
  ...
  it('disabled blocks textarea', () => {
    const w = mount(CuTextareaItem, { props: { disabled: true, value: 'locked' } })
    expect(w.find('textarea').attributes('disabled')).toBeDefined()
    expect(w.find('.cu-textarea-item__clear').exists()).toBe(false)
  })
})
...
describe('CuSteps', () => {
  ...
  it('renders status classes per current', () => {
    const w = mount(CuSteps, { props: { steps, current: 1 } })
  ...
  it('fraction current adds no-current class', () => {
    const w = mount(CuSteps, { props: { steps: steps.slice(0, 2), current: 0.5 } })
    expect(w.find('.cu-steps').classes()).toContain('no-current')
    expect(w.findAll('.step-wrapper')[0].classes()).toContain('reached')
  })

  it('current change updates progress', async () => {
    const w = mount(CuSteps, { props: { steps, current: 0 } })
  ...
  it('transition mode defers progress via timers', async () => {
    vi.useFakeTimers()
    const w = mount(CuSteps, { props: { steps, current: 0, transition: true } })
...
describe('CuTabs / CuTabBar / CuTabPane', () => {
  ...
  it('defaults to first pane and registers panes', async () => {
    const w = mountTabs()
    await settle(40)
    const tabs = w.findComponent({ name: 'cu-tabs' })
    expect((tabs.vm as unknown as { currentName: unknown }).currentName).toBe('a')
    expect(w.find('.cu-tab-bar-item.is-active').text()).toBe('标签一')
    expect(w.findAll('.cu-tab-pane')[0].isVisible()).toBe(true)
    expect(w.findAll('.cu-tab-pane')[1].isVisible()).toBe(false)
  })

  it('clicking a tab switches panes and emits', async () => {
    const w = mountTabs()
    await settle(40)
    await w.findAll('.cu-tab-bar-item')[1].trigger('click')
    expect(w.find('.cu-tab-bar-item.is-active').text()).toBe('标签二')
    expect(w.findAll('.cu-tab-pane')[1].isVisible()).toBe(true)
    expect(w.findComponent({ name: 'cu-tabs' }).emitted('change')).toBeTruthy()
  })

  it('disabled tab does not switch', async () => {
    const w = mountTabs()
    await settle(40)
    await w.findAll('.cu-tab-bar-item')[2].trigger('click')
    expect(w.find('.cu-tab-bar-item.is-active').text()).toBe('标签一')
  })
  ...
  it('model-value selects initial tab', async () => {
    const w = mount({
      ...
      template: `<cu-tabs model-value="b">
        <cu-tab-pane label="L1" name="a">1</cu-tab-pane>
        <cu-tab-pane label="L2" name="b">2</cu-tab-pane>
      </cu-tabs>`,
    })
    await settle(40)
    expect(w.find('.cu-tab-bar-item.is-active').text()).toBe('L2')
  })

  it('tab-bar disabled item blocks change', async () => {
    const w = mount(CuTabBar, {
      props: {
        items: [
          { name: 'a', label: 'A' },
    ...
    expect(w.find('.cu-tab-bar-item.is-disabled').exists()).toBe(true)
  })
  ...
  it('tab-bar string inkLength renders numeric width', async () => {
    ...
    const ink = w.find('.cu-tab-bar-ink')
