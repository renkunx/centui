/**
 * CuAmount 行为清单（自 v2 components/amount spec/源码提取）：
...
import { CuAmount } from '../../src'

describe('CuAmount', () => {
  it('formats with default precision 2 and rounds up', () => {
    const wrapper = mount(CuAmount, { props: { value: 1234.56 } })
    expect(wrapper.text()).toBe('1234.56')
    expect(wrapper.classes()).toContain('numerical')

    const up = mount(CuAmount, { props: { value: 1.005 } })
    expect(up.text()).toBe('1.01')
  })

  it('treats negative precision as 0 and floors when isRoundUp is false', () => {
    expect(mount(CuAmount, { props: { value: 12.34, precision: -1 } }).text()).toBe('12')
    expect(
      mount(CuAmount, { props: { value: 1.239, precision: 2, isRoundUp: false } }).text(),
    ).toBe('1.23')
  })

  it('groups integer part with separator and keeps decimals', () => {
    const wrapper = mount(CuAmount, { props: { value: 1234567.891, hasSeparator: true } })
    expect(wrapper.text()).toBe('1,234,567.89')

    const space = mount(CuAmount, {
      props: { value: 1234567.89, hasSeparator: true, separator: ' ' },
    })
    expect(space.text()).toBe('1 234 567.89')

    const negative = mount(CuAmount, { props: { value: -1234567.89, hasSeparator: true } })
    expect(negative.text()).toBe('-1,234,567.89')
  })

  it('renders chinese capital amount', () => {
    const wrapper = mount(CuAmount, { props: { value: 1234.56, isCapital: true } })
    expect(wrapper.text()).toBe('壹仟贰佰叁拾肆元伍角陆分')
    expect(wrapper.classes()).not.toContain('numerical')

    expect(mount(CuAmount, { props: { value: 0, isCapital: true } }).text()).toBe('零元整')
    expect(mount(CuAmount, { props: { value: -12.34, isCapital: true } }).text()).toBe(
      '负壹拾贰元叁角肆分',
    )
  })

  it('animates value changes when isAnimated', async () => {
    const wrapper = mount(CuAmount, { props: { value: 100, isAnimated: true, duration: 100 } })
