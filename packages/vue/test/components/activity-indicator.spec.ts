/**
 * MdActivityIndicator 行为清单（自 v2 components/activity-indicator spec/源码提取）：
...
import { MdActivityIndicator, MdActivityIndicatorRolling } from '../../src'

describe('MdActivityIndicator', () => {
  it('renders roller with geometry from size', () => {
    const wrapper = mount(MdActivityIndicator, { props: { type: 'roller', size: 70 } })
  ...
  it('overrides stroke width via width prop', () => {
    const wrapper = mount(MdActivityIndicatorRolling, { props: { size: 70, width: 10 } })
  ...
  it('renders progress state when process provided', () => {
    const wrapper = mount(MdActivityIndicatorRolling, { props: { size: 70, process: 0.5 } })
  ...
  it('renders spinner with dark color by default', () => {
    const dark = mount(MdActivityIndicator, { props: { type: 'spinner' } })
    expect(dark.find('.md-activity-indicator-spinning.dark').exists()).toBe(true)

    const light = mount(MdActivityIndicator, { props: { type: 'spinner', color: 'light' } })
    expect(light.find('.md-activity-indicator-spinning.dark').exists()).toBe(false)

    const sized = mount(MdActivityIndicator, { props: { type: 'spinner', size: 40 } })
    expect(sized.find('.md-activity-indicator-svg').attributes('style')).toContain('width: 40px')
  })

  it('keeps explicit color for roller', () => {
    const wrapper = mount(MdActivityIndicator, { props: { type: 'roller', color: '#fc0' } })
    expect(wrapper.find('circle.stroke').attributes('stroke')).toBe('#fc0')
  })

  it('renders default slot as text with vertical layout', () => {
    const wrapper = mount(MdActivityIndicator, {
      props: { vertical: true },
      slots: { default: '加载中' },
    })
    expect(wrapper.find('.indicator-container.vertical').exists()).toBe(true)
    expect(wrapper.find('.md-activity-indicator-text').text()).toBe('加载中')
  })

  it('renders carousel with three circles', () => {
    const wrapper = mount(MdActivityIndicator, { props: { type: 'carousel', size: 30 } })
