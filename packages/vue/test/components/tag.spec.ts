/**
 * CuTag 行为清单（自 v2 components/tag spec/源码提取）：
...
import { CuTag } from '../../src'

describe('CuTag', () => {
  it('renders default class combination and slot', () => {
    const wrapper = mount(CuTag, { slots: { default: '标签' } })
    const inner = wrapper.find('.cu-tag > div')
  ...
  it('renders quarter shape with bg layer', () => {
    const wrapper = mount(CuTag, { props: { shape: 'quarter' }, slots: { default: 'Q' } })
  ...
  it('applies fill color and font color styles', () => {
    const fill = mount(CuTag, { props: { type: 'fill', fillColor: '#fc9153' } })
    expect(fill.find('.type-fill').attributes('style')).toContain('background: rgb(252, 145, 83)')

    const ghost = mount(CuTag, { props: { type: 'ghost', fontColor: '#f00' } })
    ...
    const filled = mount(CuTag, { props: { type: 'fill', fontColor: '#0f0' } })
  ...
  it('computes circle padding from element height on mount', async () => {
    const wrapper = mount(CuTag, { props: { shape: 'circle' } })
    await flushPromises()
  ...
  it('resets sharp corner radius for circle shape', async () => {
    const wrapper = mount(CuTag, { props: { shape: 'circle', sharp: 'top-left' } })
    await flushPromises()
  ...
  it('supports bubble and fillet shapes', () => {
    expect(
      mount(CuTag, { props: { shape: 'bubble' } })
        .find('.shape-bubble')
        .exists(),
    ).toBe(true)
    expect(
      mount(CuTag, { props: { shape: 'fillet' } })
        .find('.shape-fillet')
        .exists(),
    ).toBe(true)
  })
