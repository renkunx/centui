/**
 * CuPopup 行为清单（自 v2 components/popup spec/源码提取，v-model → modelValue）：
...
describe('CuPopup', () => {
  ...
  it('opens with mask and position classes, box carries transition name', async () => {
    ...
    expect(wrapper.find('.cu-popup-box').classes()).toContain('cu-fade')
    expect(wrapper.attributes('style')).not.toContain('display: none')
    expect(wrapper.find('.cu-popup-mask').attributes('style')).not.toContain('display: none')

    const bottom = mount(CuPopup, { props: { modelValue: true, position: 'bottom' } })
    await nextTick()
    expect(bottom.find('.cu-popup-box').classes()).toContain('cu-slide-up')

    const custom = mount(CuPopup, {
      props: { modelValue: true, position: 'center', transition: 'cu-fade-bounce' },
    })
    await nextTick()
    expect(custom.find('.cu-popup-box').classes()).toContain('cu-fade-bounce')
  })

  it('omits with-mask class when hasMask is false', async () => {
    const wrapper = mount(CuPopup, { props: { modelValue: true, hasMask: false } })
    await nextTick()
    expect(wrapper.classes()).not.toContain('with-mask')
  })

  it('emits show lifecycle events when opened', () => {
    const wrapper = mount(CuPopup, { props: { modelValue: true } })
  ...
  it('closes on mask click when maskClosable', async () => {
    const wrapper = mount(CuPopup, { props: { modelValue: true } })
    await wrapper.find('.cu-popup-mask').trigger('click')
  ...
  it('does nothing on mask click when maskClosable is false', async () => {
    const wrapper = mount(CuPopup, { props: { modelValue: true, maskClosable: false } })
    await wrapper.find('.cu-popup-mask').trigger('click')
  ...
  it('binds and unbinds touchmove for preventScroll', async () => {
    ...
    mount(CuPopup, { props: { modelValue: true, preventScroll: true } })
    ...
    const wrapper = mount(CuPopup, { props: { modelValue: true, preventScroll: true } })
