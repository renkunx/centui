...
describe('MdSelector', () => {
  ...
  it('okText requires confirm; confirm emits selected item', async () => {
    const w = mount(MdSelector, { props: { data, modelValue: true, okText: '确定' } })
    await settle(80)
    await w.findAll('.md-radio-item')[1].trigger('click')
    expect(w.emitted('choose')?.length).toBe(1)
    expect(JSON.stringify(w.html())).not.toContain('display: none') // 仍打开
    await w.find('.md-popup-title-bar .md-popup-confirm').trigger('click')
    expect(w.emitted('confirm')?.length).toBe(1)
  })

  it('multi mode accumulates and confirm emits array', async () => {
    const w = mount(MdSelector, {
      props: { data, modelValue: true, multi: true, okText: '确定', defaultValue: ['1'] },
    })
    await settle(80)
    // 已默认勾选第一项；再勾选第二项
    await w.findAll('.md-check-item')[1].trigger('click')
    await w.find('.md-popup-title-bar .md-popup-confirm').trigger('click')
  ...
  it('cancel resets selection and emits cancel', async () => {
    ...
    await settle(80)
    await w.findAll('.md-radio-item')[1].trigger('click')
    await w.find('.md-popup-title-bar .md-popup-cancel').trigger('click')
...
describe('MdDropMenu', () => {
  ...
  it('renders bar with default-selected text', async () => {
    const w = mount(MdDropMenu, { props: { data, defaultValue: ['2'] } })
    await settle(30)
  ...
  it('bar click opens popup, list click selects and closes', async () => {
    ...
    const w = mount(MdDropMenu, { props: { data, onChange } })
    await settle(50)
    await w.findAll('.bar-item')[1].trigger('click')
    await settle(60)
    expect(w.find('.md-popup-box').isVisible()).toBe(true)
    expect(w.findAll('.bar-item')[1].classes()).toContain('active')
    // 选中列表项
    await w.findAll('.md-radio-item')[0].trigger('click')
  ...
  it('disabled bar item blocks open', async () => {
    const w = mount(MdDropMenu, { props: { data } })
    await w.findAll('.bar-item')[2].trigger('click')
    await settle(40)
    expect(w.find('.md-popup-box').isVisible()).toBe(false)
  })

  it('exposes selected values', async () => {
    const w = mount(MdDropMenu, { props: { data, defaultValue: ['', '4'] } })
    await settle(50)
...
describe('MdTabPicker', () => {
  ...
  it('defaultValue derives panes and labels', async () => {
    const w = mount(MdTabPicker, {
      props: { data: cascadeData, defaultValue: ['zj', 'hz'] },
    })
    await settle(50)
    // 默认值下生成两级 pane
    expect(w.findAll('.md-tab-pane')).toHaveLength(2)
  })
  ...
  it('select leaf emits change and closes', async () => {
    ...
    await settle(80)
    // 打开时只有一级 pane
    expect(w.findAll('.md-tab-pane')).toHaveLength(1)
    // 选浙江 → 二级 pane 出现
    await w.findAll('.md-radio-item')[0].trigger('click')
    await settle(80)
    ...
    const panes = w.findAll('.md-tab-pane')
    await panes[1].find('.md-radio-item').trigger('click')
    await settle(500)
