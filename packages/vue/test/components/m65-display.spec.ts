...
describe('CuBill', () => {
  it('renders title and no with default structure', () => {
    ...
    expect(w.find('.cu-bill').exists()).toBe(true)
    expect(w.find('.cu-bill-title').text()).toBe('借款电子凭证')
    expect(w.find('.cu-bill-no').text()).toBe('NO.12345689')
    expect(w.find('.cu-bill-neck').exists()).toBe(true)
  ...
  it('header slot replaces title/no; footer slot renders', () => {
    ...
    expect(w.find('.cu-bill-footer .ftr').exists()).toBe(true)
  })

  it('content watermark renders canvas', () => {
    const w = mount(CuBill, { props: { waterMark: 'centui' }, slots: { default: 'x' } })
...
describe('CuImageViewer', () => {
  ...
  it('closed by default; value opens with items', async () => {
    const w = mount(CuImageViewer, { props: { list } })
    // VTU isVisible 在未挂载树上不可靠，用 v-show 的 style 断言
    expect(w.find('.cu-image-viewer').attributes('style')).toContain('display: none')
    await w.setProps({ modelValue: true })
    await settle(80)
    expect(w.find('.cu-image-viewer').attributes('style')).not.toContain('display: none')
  ...
  it('initialIndex sets start position', async () => {
    const w = mount(CuImageViewer, { props: { list, modelValue: true, initialIndex: 1 } })
    await settle(80)
    expect(w.find('.viewer-index').text()).toBe('2/2')
  })

  it('viewer click closes and emits update', async () => {
    const w = mount(CuImageViewer, { props: { list, modelValue: true } })
    await settle(80)
    await w.find('.cu-image-viewer').trigger('click')
    expect(w.emitted('update:modelValue')?.at(-1)).toEqual([false])
  })

  it('object list items pass url/alt', async () => {
    const w = mount(CuImageViewer, {
...
describe('CuCaptcha', () => {
  it('inline mode renders content and codebox', async () => {
    ...
    await settle(50)
    expect(w.find('.cu-captcha-title').text()).toBe('输入验证码')
    expect(w.find('.cu-captcha-message').text()).toBe('短信验证码已发送')
    expect(w.find('.cu-codebox').exists()).toBe(true)
  })
  ...
  it('auto countdown starts and resets', async () => {
    ...
    expect(w.find('.cu-captcha-btn').text()).toBe('重发(3s)')
    expect((w.find('.cu-captcha-btn').element as HTMLButtonElement).disabled).toBe(true)
    await vi.advanceTimersByTimeAsync(3100)
    expect(w.find('.cu-captcha-btn').text()).toBe('重新发送')
    vi.useRealTimers()
  })

  it('submit emits code', async () => {
    const onSubmit = vi.fn()
    const w = mount(CuCaptcha, { props: { isView: true, maxlength: 4, onSubmit } })
    await settle(40)
  ...
  it('halfScreen mode renders popup skeleton and dialog mode too', () => {
    const w = mount(CuCaptcha, { props: { type: 'halfScreen', modelValue: true } })
    // 半屏模式包含 title bar
