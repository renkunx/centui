/**
 * CuStepper 行为清单（自 v2 components/stepper test/index.spec.js 提取，v-model → modelValue）：
...
import { CuStepper } from '../../src'

function mountHost(value: number) {
  const Host = defineComponent({
    components: { CuStepper },
    data: () => ({ value }),
    template: `<CuStepper v-model="value" :min="0" />`,
  })
  return mount(Host)
}
...
describe('CuStepper', () => {
  ...
  it('falls back to defaultValue when modelValue is falsy', async () => {
    const wrapper = mount(CuStepper, { props: { defaultValue: 3, min: 0 } })
    await flushPromises()
  ...
  it('steps by step size with float precision safety', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 1, step: 2 } })
    await wrapper.find('.cu-stepper-button-add').trigger('click')
    expect((wrapper.find('input').element as HTMLInputElement).value).toBe('3')

    const float = mount(CuStepper, { props: { modelValue: 1, step: 0.1 } })
    await float.find('.cu-stepper-button-add').trigger('click')
    expect((float.find('input').element as HTMLInputElement).value).toBe('1.1')
  })

  it('disables reduce/add buttons at boundaries', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 1, min: 0, max: 2 } })
    await flushPromises()
    expect(wrapper.find('.cu-stepper-button-reduce').classes()).not.toContain('disabled')

    await wrapper.find('.cu-stepper-button-reduce').trigger('click')
    expect((wrapper.find('input').element as HTMLInputElement).value).toBe('0')
    expect(wrapper.find('.cu-stepper-button-reduce').classes()).toContain('disabled')
    ...
    expect(wrapper.find('.cu-stepper-button-add').classes()).toContain('disabled')
  })

  it('blocks operations when disabled', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 1, disabled: true } })
  ...
  it('supports readOnly and isInteger', async () => {
    const readOnly = mount(CuStepper, { props: { readOnly: true } })
    expect(readOnly.find('input').attributes('readonly')).toBeDefined()

    const integer = mount(CuStepper, { props: { modelValue: 3, isInteger: true } })
  ...
  it('emits update:modelValue and change on value change', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 1 } })
    await wrapper.find('.cu-stepper-button-add').trigger('click')
  ...
  it('emits increase and decrease with diff', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 0 } })
    await wrapper.find('.cu-stepper-button-add').trigger('click')
  ...
  it('formats raw input and syncs value', async () => {
    ...
    const wrapper = mount(CuStepper, { props: { modelValue: 1, isInteger: true } })
  ...
  it('clamps current value on blur', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 1, min: 2, max: 5 } })
    await wrapper.find('input').trigger('blur')
    expect((wrapper.find('input').element as HTMLInputElement).value).toBe('2')
  })

  it('clamps when min/max props change', async () => {
    const wrapper = mount(CuStepper, { props: { modelValue: 3 } })
    await flushPromises()
  ...
  it('warns when min is larger than max', () => {
    ...
    mount(CuStepper, { props: { min: 5, max: 3 } })
