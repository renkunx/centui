...
import { MdDropMenu, MdSelector, MdTabPicker } from '../../src'
...
describe('MdSelector', () => {
  ...
  it('closed by default, opens via value', async () => {
    const { container, rerender } = render(<MdSelector data={data} />)
    expect(container.querySelector('.md-popup-box')?.getAttribute('style')).toContain('display: none')
    rerender(<MdSelector data={data} value />)
    await flush(80)
  ...
  it('immediate choose closes and emits choose + onChange', async () => {
    ...
    const { container } = render(<MdSelector data={data} value onChange={onChange} onChoose={onChoose} />)
    await flush(80)
    fireEvent.click(container.querySelectorAll('.md-radio-item')[0])
    await flush(60)
  ...
  it('okText requires confirm', async () => {
    ...
    const { container } = render(
      <MdSelector data={data} value okText="确定" onConfirm={onConfirm} />,
    )
    await flush(80)
  ...
  it('multi mode accumulates and confirm emits array', async () => {
    const onConfirm = vi.fn()
    const { container } = render(
      <MdSelector data={data} value multi okText="确定" defaultValue={['1']} onConfirm={onConfirm} />,
    )
    await flush(80)
    fireEvent.click(container.querySelectorAll('.md-check-item')[1])
    await flush(40)
    fireEvent.click(container.querySelector('.md-popup-confirm')!)
  ...
  it('cancel resets and emits cancel', async () => {
    ...
    const { container } = render(
      <MdSelector data={data} value okText="确定" cancelText="取消" onCancel={onCancel} />,
    )
    await flush(80)
    fireEvent.click(container.querySelectorAll('.md-radio-item')[1])
  ...
  it('hideTitleBar hides title bar', () => {
    const { container } = render(<MdSelector data={data} value hideTitleBar />)
    void container
    // hideTitleBar 且无确认 → 标题栏不渲染（v-show 契约：React 条件渲染）
    const { container: c2 } = render(<MdSelector data={data} value hideTitleBar />)
    expect(c2.querySelector('.md-popup-title-bar')).toBeNull()
  })
})
...
describe('MdDropMenu', () => {
  ...
  it('renders bar with default-selected text', () => {
    const { container } = render(<MdDropMenu data={data} defaultValue={['2']} />)
  ...
  it('bar click opens popup, list click selects and reports change', async () => {
    const onChange = vi.fn()
    const { container } = render(<MdDropMenu data={data} onChange={onChange} />)
    await flush(40)
  ...
  it('disabled bar item blocks open', async () => {
    const { container } = render(<MdDropMenu data={data} />)
  ...
  it('exposes selected values', () => {
    ...
    render(<MdDropMenu data={data} defaultValue={['', '4']} ref={ref as never} />)
    void ref.current
    expect(ref.current).not.toBeNull()
  })
})

describe('MdTabPicker', () => {
  const cascadeData = {
    name: 'level1',
    label: '一级',
    options: [
  ...
  it('defaultValue derives panes', () => {
    const { container } = render(<MdTabPicker data={cascadeData} defaultValue={['zj', 'hz']} />)
    expect(container.querySelectorAll('.md-tab-pane')).toHaveLength(2)
  })
  ...
  it('select leaf emits change and closes', async () => {
    ...
    const onInput = vi.fn()
    const { container } = render(
      <MdTabPicker data={cascadeData} value onChange={onChange} onSelect={onSelect} onInput={onInput} />,
    )
    await flush(100)
    expect(container.querySelectorAll('.md-tab-pane')).toHaveLength(1)
    // 一级选浙江
    fireEvent.click(container.querySelectorAll('.md-radio-item')[0])
    await flush(100)
    ...
    fireEvent.click(container.querySelectorAll('.md-tab-pane')[1].querySelector('.md-radio-item')!)
    await flush(600)
